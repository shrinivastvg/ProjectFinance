...
let startTime;
let endTime;
let initialCost = 10000000; // ₹1 crore
let initialTime = 6; // 6 months
let totalQuestions = 30; // Replace with your actual total questions
let costDecrement = initialCost / totalQuestions; // Amount to reduce or increase per question
let timeDecrement = initialTime / totalQuestions; // Amount to reduce or increase per question
...
function handleOptionSelect(selectedOption, correctAnswer, element) {
    ...
    document.getElementById("project-cost").innerText = Math.max(initialCost, 0).toFixed(2); // Prevent negative cost
    document.getElementById("project-time").innerText = Math.max(initialTime, 0).toFixed(2); // Prevent negative time
